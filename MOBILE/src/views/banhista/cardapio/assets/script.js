...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Abrir o modal ao clicar no produto
    produtoDiv.addEventListener("click", () => {
        modalProduto.showModal();
    });
    ...
    const addSacola = document.getElementById('adicionarSacola')
    
    addSacola.addEventListener('click', () => {
        const notificacao = document.getElementById('notificacao');
        modalProduto.close();
        notificacao.style.display = 'flex';
        ...
        setTimeout(() => {
            notificacao.style.animation = 'fecharNtf 0.2s linear';
            notificacao.addEventListener('animationend', () => {
                notificacao.style.display = 'none';
            }, { once: true });
        }, 2000);
